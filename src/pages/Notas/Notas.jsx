// =============================================================
// PAGINA DE NOTAS - pages/Notas/Notas.jsx
// =============================================================
...
import { getEstudiantesVinculados, getEstudiantesPorDocente, getAsignaturas } from '../../services/usuarioService'
...
export default function Notas() {
  ...
  const onRegistrar = async (data) => {
    try {
      ...
      toast.error('No se pudo registrar la calificacion')
    }
  }
  ...
  return (
    <div className="page-content">
      <PageHeader
        ...
        subtitle={esDocente ? 'Registra y consulta las calificaciones de tus estudiantes' : 'Consulta el rendimiento academico'}
        action={esDocente && seleccionado && (
      ...
      {estudianteActual ? (
        ...
        <p className="text-suave empty-state">No hay estudiantes disponibles para este usuario.</p>
      )}

      <div className="notas__chart card">
        <h3><RiBarChartBoxLine /> Promedio por asignatura</h3>
        {promedios.length === 0 ? (
          <p className="text-suave">Aun no hay calificaciones registradas.</p>
        ) : (
      ...
      <div className="data-table-wrap">
        <table className="data-table">
          <thead>
            <tr><th>Fecha</th><th>Asignatura</th><th>Evaluacion</th><th>Nota</th></tr>
          </thead>
          <tbody>
            {notas.length === 0 ? (
              <tr><td colSpan={4} className="text-suave">Sin registros aun.</td></tr>
            ) : notas.map((n) => (
              <tr key={n.id}>
                <td>{n.fecha}</td>
                <td>{n.asignatura?.nombre || '-'}</td>
                <td>{n.descripcion}</td>
                <td><span className={'nota-pill ' + (n.valor < 4 ? 'nota-pill--baja' : 'nota-pill--ok')}>{n.valor.toFixed(1)}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {modalAbierto && (
        <Modal title="Registrar calificacion" onClose={() => setModalAbierto(false)}>
          <form onSubmit={handleSubmit(onRegistrar)}>
            ...
            <div className="form-group">
              ...
              <input type="number" step="0.1" min="1" max="7" {...register('valor', { required: 'Ingresa la nota', min: { value: 1, message: 'Minimo 1.0' }, max: { value: 7, message: 'Maximo 7.0' } })} />
